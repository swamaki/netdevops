...
class Fortigate:
    ...
    def ApiAdd(self, url, data=None):
        req = self.s.post(
            self.api_url + url, params={"vdom": self.vdom}, data=repr(data)
        )
        return req.status_code
    ...
    def AddSystemAdmin(
        self, name, password, profile="prof_admin", remote_auth="disable"
    ):
        """
    ...
    def AddVlanInterface(
        self, name, interface, vlanid, ip_mask, vdom, mode="none", allowaccess=""
    ):
        """
        ...
        req = self.s.post(
            self.api_url + url, params={"vdom": "root"}, data=repr(payload)
        )
        # print 'ApiAdd text:', req.text
        return req.status_code

    def AddVlanInterfaceIdempotent(
        self, name, interface, vlanid, ip_mask, vdom, mode, allowaccess
    ):
        """
        ...
        if not (self.Exists("cmdb/system/interface/", objects)):
            # object does not exist, create it
            return self.AddVlanInterface(
                name, interface, vlanid, ip_mask, vdom, mode, allowaccess
            )
        else:
            # object already Exist
            return 200

    def SetVlanInterface(
        self, name, interface, vlanid, ip_mask, vdom, mode="none", allowaccess=""
    ):
        """
    ...
    def AddFwAddressRange(
        self, name, start_ip, end_ip, associated_interface="", comment=""
    ):
        """
    ...
    def DelFwPolicy(
        self, srcintf="any", dstintf="any", srcaddr="all", dstaddr="all", service="ALL"
    ):
        """
    ...
    def AddFwServiceIdempotent(
        ...
        return_code = self.AddFwService(
            name, tcp_portrange, udp_portrange, protocol, fqdn, iprange, comment
        )
        if return_code != 200:
            # creation failed, check to see if the object already exists
            objects = [
    ...
    def AddTrafficShaperIdempotent(
        self,
        name,
        per_policy,
        priority,
        ...
        Http status code: 200 if ok, 4xx if an error occurs
        """
        return_code = self.AddTrafficShaper(
            name,
            per_policy,
            priority,
            guaranteed_bandwidth,
    ...
    def AddFwVIPidempotent(
        ...
        return_code = self.AddFwVIP(
            name, extip, extintf, mappedip, portforward, extport, mappedport, comment
        )
        if return_code != 200:
            # creation failed, check to see if the object already exists
            objects = [["name", name]]
    ...
    def AddFwIPpool(
        ...
        endip,
        type_pool="overload",
        internal_startip="0.0.0.0",
        internal_endip="0.0.0.0",
        arp_reply="enable",
        block_size="128",
    ...
    def AddVPNipsecPhase1Idempotent(
        self,
        name,
        interface,
        remote_gw,
    ...
    def AddVPNipsecPhase2Idempotent(
        self,
        name,
        phase1name,
        local_addr_type,
    ...
    def DelVPNipsecPhase1(self, name):
        ...
        Http status code: 200 if ok, 4xx if an error occurs
        """
        payload = {"json": {"name": "phase1-interface"}}
    ...
    def DelVPNipsecPhase2(self, name):
        ...
        Http status code: 200 if ok, 4xx if an error occurs
        """
        payload = {"json": {"name": "phase2-interface"}}
