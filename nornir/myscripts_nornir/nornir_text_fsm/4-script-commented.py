...
def get_facts(task):
    ...
    for x in range(0, 8):
        ...
        if "trunk" in task.host["facts"][x]["admin_mode"]:
            trunk_description = task.run(
                netmiko_send_config, config_commands=trunk_commands
            )
        # Condition 2: if the word "access" is in the targeted key-pair value output, use Netmiko to send the access_commands
            ...
            access_description = task.run(
                netmiko_send_config, config_commands=access_commands
            )
        # Condition 3: if the word "down" is in the targeted key-pair value output, use Netmiko to send the shut_commands
            ...
            shut_description = task.run(
                netmiko_send_config, config_commands=shut_commands
            )
